...
function generateSourceFiles(sourceFiles, filesMetaData, encoding) {
    encoding = encoding || 'utf8';
    /* * /
    console.log("\t[PUBLISH@JM2MP]: Begin");
    filesMetaData.forEach((fmd)=>{
        console.log(`\t[PUBLISH@JM2MP]: ${JSON.stringify(fmd)}`);
    });
    console.log("\t[PUBLISH@JM2MP]: End");
    / * */
    Object.keys(sourceFiles).forEach(function (file) {
        ...
        try {
            ...
            metaData = filesMetaData.find( (fmd)=>(sourceFiles[file].shortened === fmd.longname) );
        }
        catch (e) {
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    if (outputSourceFiles) {
        generateSourceFiles(sourceFiles, files, opts.encoding);
    }
